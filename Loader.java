...
public class Loader {
	...
	private static HashMap<Integer, List<Integer>> vaoCache = new HashMap<Integer, List<Integer>>();

	//for normal models with textures 
	...
	public int loadTexture(String fileName, String format){
		...
		try {
			...
			if(format.equals("png")){
				texture = TextureLoader.getTexture("PNG", new FileInputStream((fileName+ "." + format)));
			}
			else if(format.equals("jpg")){
				texture = TextureLoader.getTexture("JPG", new FileInputStream((fileName+ "." + format)));

			}
			else if(format.equals("tga")){
				texture = TextureLoader.getTexture("TGA", new FileInputStream((fileName+ "." + format)));
			}
			else if(format.equals("bmp")){
				texture = TextureLoader.getTexture("BMP", new FileInputStream((fileName+ "." + format)));
			}
	...
	/*	private int createVAO(){
		...
		vaos.add(vaoID); //to be deleted when the game is closed
	...
	private void storeDataInAttributeList(int attributeNumber, int coordinateSize, float[] data){
		...
		GL15.glBufferData(GL15.GL_ARRAY_BUFFER, buffer, GL15.GL_STATIC_DRAW); //it is static, we're not going to edit it 
	...
	private FloatBuffer storeDataInFloatBuffer(float [] data){
		...
		buffer.put(data);
		buffer.flip(); //if we flip it then it knows that it is ready to be read
		return buffer;
	}
